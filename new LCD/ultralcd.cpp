...
void kill_screen(const char* lcd_msg) {
  ...
  static void lcd_setZProbeHeight_menu() {
    ...
    if (encoderPosition) {
		...
		if(home_offset[Z_AXIS] + float((int32_t)encoderPosition) * 0.01 >= 0){ //Only allow positive offset
			 home_offset[Z_AXIS] += float((int32_t)encoderPosition) * 0.01;
   ...
   */
    ...
    if (encoderPosition) {
	  ...
	  tempZOffset+= float((int32_t)encoderPosition) * 0.01;
	  
	  current_position[Z_AXIS] -= float((int32_t)encoderPosition) * 0.01; //call sync_plan_position soon after this.
  ...
  static void lcd_prepare_menu() {
	...
	if (planner.movesplanned() || IS_SD_PRINTING) {
		...
		MENU_ITEM(function, "Probe for ZO", lcd_AutoHomeZ_menu); 
