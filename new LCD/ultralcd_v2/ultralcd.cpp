...
uint8_t lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW; // Set when the LCD needs to draw, decrements after every draw. Set to 2 in LCD routines so the LCD gets at least 1 full redraw (first redraw is partial)
  ...
  /* Helper macros for menus */
    ...
    #define ENCODER_STEPS_PER_MENU_ITEM 4 //Kalle 1 hole step on knob = 4 encoder steps
  #endif
...
void kill_screen(const char* lcd_msg) {
  ...
  /**
   *
   * "Home Z" submenu //custom function created by Kalle:
   ...
   */
	...
	//encoderPosition=0;
	lcd_implementation_drawedit(PSTR("Probe Z?"), HomeZText);
  ...
  static void lcd_setZProbeHeight_menu() {
    ...
    if(encoderPosition % 4 == 0 && home_offset[Z_AXIS] + float((int32_t)encoderPosition) * 0.01/4 >= 0){ //Only allow positive offset
       home_offset[Z_AXIS] += float((int32_t)encoderPosition) * 0.01/4;
       current_position[Z_AXIS] += float((int32_t)encoderPosition) * 0.01/4;
       encoderPosition = 0;       
    }
  ...
  float tempZOffset =0; //to keep track of the total temp offset!
    ...
    if(encoderPosition % 4 == 0){
      tempZOffset+= float((int32_t)encoderPosition) * 0.01/4;
    
      current_position[Z_AXIS] -= float((int32_t)encoderPosition) * 0.01/4; //call sync_plan_position soon after this.
      encoderPosition = 0;
    }
  ...
  if (planner.movesplanned() || IS_SD_PRINTING) {
    ...
    MENU_ITEM(submenu2, "Probe for ZO", lcd_AutoHomeZ_menu); 
    ...
    //SD-card: 
      ...
      if (card.isFileOpen()) {
        ...
        MENU_ITEM(submenu2, MSG_CARD_MENU, lcd_sdcard_menu); //Submenu2 might be bad if empty sdcard!?
        #if !PIN_EXISTS(SD_DETECT)
        MENU_ITEM(gcode, MSG_CNG_SDCARD, PSTR("M21"));  // SD-card changed by user
        #endif
      }
  ...
  */ 
  ...
  static void _lcd_move_xyz(const char* name, AxisEnum axis, float min, float max) {
    ...
    if (encoderPosition) {
      refresh_cmd_timeout();
    if(encoderPosition%4==0){ //Kalle to make one knob-step equal one movescale!
      current_position[axis] += float((int32_t)encoderPosition) * move_menu_scale/4; //Kalle /4
